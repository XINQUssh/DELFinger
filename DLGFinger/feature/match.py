...
def scoreFeatureMatchArea(featureSrc, featureMatch):
	...
	listSrcX, listSrcY = listFeature(featureSrc)
	listMatchX, listMatchY = listFeature(featureMatch)
...
def finalTransforMatMeans(featureSrc, featureMatch, iter, coupleNum):
	...
	for i in range(iter):
		srcPoints,matchPoints = randomPoints(featureSrc, featureMatch, coupleNum)
	...
	for i in range(3*(coupleNum-1)):
		...
		meansFitMat.append(calMeans(tempData))
...
def finalTransforMatRansac(featureSrc, featureMatch):
	...
	model = LinearLeastSquareModel(inputCols, outputCols)
	ransacFitMat, ransacData = ransac(allData, model, len(featureSrc)//2, 100, 250, len(featureSrc)//4)
...
def match(text_path,pic1_path,pic2_path):
	...
	# read data from txt
	featureSrc, featureMatch = readTxt('1151')
