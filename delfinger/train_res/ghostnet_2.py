...
class GhostNet(nn.Module):
    def __init__(self, atten=False, freeze=False, num_classes=1000, width=1.0):
        ...
        self.attn = SpatialAttention2d(in_c=960, act_fn='relu')
        self.ae = AutoEncoder(in_c=960)
        ...
        nn.Conv2d(input_channel, output_channel, 3, 2, 0, bias=True),
        nn.BatchNorm2d(output_channel),
        nn.ReLU(inplace=True),
        nn.Conv2d(input_channel, output_channel, 3, 2, 1, bias=True),
        nn.BatchNorm2d(output_channel),
        nn.ReLU(inplace=True)
        )
        ...
        self.attn_classifier = nn.Linear(960, num_classes)

        self.module_dict = {}
        self.module_dict['base'] = nn.Sequential(self.conv_stem, self.bn1, self.act1, self.blocks)
        self.module_dict['pool'] = nn.Sequential(self.global_block, self.global_pool, self.Flatten, self.classifier)
...
def ghostnet(ncls, **kwargs):
    """
    Constructs a GhostNet model
    """
    return GhostNet(atten = False, freeze=False, num_classes = ncls, **kwargs)

def ghostnet_frozen(ncls, **kwargs):
    return GhostNet(atten = True, freeze=True, num_classes = ncls, **kwargs)
