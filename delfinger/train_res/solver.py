...
class Solver(object):
    def __init__(self, config, model):
        ...
        #self.metric_fc = ArcMarginProduct(512, ncls, s=30, m=1, easy_margin=False)
    ...
    def __solve__(self, mode, epoch, dataloader, attn = True):
        '''solve
        mode: train / val
        '''
        ...
        for batch_idx, (inputs, labels) in enumerate(dataloader):
            ...
            # forward
            if self.config.stage in ['keypoint']:
                outputs, attn_f, attn_ae = self.model(inputs)
                loss = self.criterion(outputs, labels) + 10 * self.criterion2(attn_ae, attn_f)
            else:
                outputs = self.model(inputs)
                #outputs = self.metric_fc(feature, labels)
