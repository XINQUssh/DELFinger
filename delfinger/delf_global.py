    ...
    def __init__(
        ...
        if self.arch in ['resnet18', 'resnet34', 'resnet50', 'resnet101', 'resnet152']:
            ...
            if self.stage in ['finetune']:
                ...
                self.__register_module__('ae', AutoEncoder(in_c=in_c))
            ...
            if self.stage in ['finetune']:
                ...
                __load_weights_from__(self.module_dict, load_dict, modulenames=['base','layer4','pool','logits'])
    ...
    def forward(self, x):
        if self.stage in ['finetune']:
            ...
            if self.use_l2_normalized_feature:
                ...
                attn_x = x
            attn_x_ori = x
            _, attn_x = self.__forward_and_save__(attn_x_ori, 'ae')
            attn_score = self.__forward_and_save__(x, 'attn')
            x = self.__forward_and_save__([attn_x, attn_score], 'pool')
            x = self.__forward_and_save__(x, 'logits')
            
            return x, attn_x, attn_x_ori
