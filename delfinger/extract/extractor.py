...
def __build_delf_config__(data):
    ...
    parser.add_argument('--ncls', type=int, default=786)
...
if __name__ == "__main__":
    ...
    MODEL_NAME = 'res18_mix_debase_3'
    
    #LOAD_FROM = '../train/repo/{}/keypoint/ckpt/fix.pth.tar'.format(MODEL_NAME)
    LOAD_FROM = '../train_res/res18_mix_debase_1/keypoint/ckpt/fix.pth.tar'
    PCA_PARAMETERS_PATH = './output/pca/{}/pca.h5'.format(MODEL_NAME)

    os.makedirs('./output/pca/{}'.format(MODEL_NAME))
    ...
    if MODE.lower() in ['pca']:
        OUTPUT_PATH = './output/pca/res18_mix_enhance_2'
        INPUT_PATH = '../notebook/pca'
        extractor.extract(INPUT_PATH, OUTPUT_PATH)
    
    elif MODE.lower() in ['delf']:
        # query
        INPUT_PATH = '/home/fingerprint/Data/xiaomi_test_mix_debase/normal/query'
        OUTPUT_PATH = './output/delf.batch/{}/query.delf'.format(MODEL_NAME)
        extractor.extract(INPUT_PATH, OUTPUT_PATH)
        # index
        INPUT_PATH = '/home/fingerprint/Data/xiaomi_test_mix_debase/normal/gallery'
        OUTPUT_PATH = './output/delf.batch/{}/index.delf'.format(MODEL_NAME)
        extractor.extract(INPUT_PATH, OUTPUT_PATH)
