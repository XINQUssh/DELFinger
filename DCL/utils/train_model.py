...
def train(Config,
    ...
    for epoch in range(start_epoch,epoch_num-1):
        ...
        for batch_cnt, data in enumerate(data_loader['train']):
            ...
            if Config.use_dcl:
                print('Epoch: {:-3d} / {:d}   step: {:-5d} / {:d} loss=ce_loss+swap_loss+law_loss: {:6.4f} = {:6.4f} + {:6.4f} + {:6.4f} '
                        .format(epoch, epoch_num-start_epoch-1, step%checkpoint, train_epoch_step, loss.detach().item(), ce_loss.detach().item(), swap_loss.detach().item(), law_loss.detach().item()), flush=True)
            if Config.use_backbone:
                print('Epoch: {:-3d} / {:d}   step: {:-5d} / {:d} loss=ce_loss+swap_loss+law_loss: {:6.4f} = {:6.4f} '
                          .format(epoch, epoch_num-start_epoch-1, step%checkpoint, train_epoch_step, loss.detach().item(), ce_loss.detach().item()), flush=True)
            ...
            if step % checkpoint == 0:
                rec_loss = []
                epoch_acc = running_corrects / data_loader['train'].total_item_len
                epoch_loss = running_loss / data_loader['train'].total_item_len
                print(32*'-', flush=True)
                ...
                print('train_set  Loss: {:.4f} Acc: {:.4f}'.format(epoch_loss, epoch_acc))
        ...
        #epoch_acc = running_corrects / data_loader['train'].total_item_len
        #epoch_loss = running_loss / data_loader['train'].total_item_len
        #print(' {:-3d}  train_set  Loss: {:.4f} Acc: {:.4f}'.format(epoch, epoch_loss, epoch_acc))
        #print('------------------------------------------------------------')
