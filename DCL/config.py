...
class LoadConfig(object):
    def __init__(self, args, version):
        ...
        if args.dataset == 'product':
            ...
            self.numcls = 100
        elif args.dataset == 'fp':
            self.dataset = args.dataset
            self.rawdata_root = './datasets/fingerprint/data/train_ks'
            self.anno_root = './datasets/fingerprint/anno'
            self.numcls = 209
        else:
        ...
        if 'train' in get_list:
             self.train_anno = pd.read_csv(os.path.join(self.anno_root, 'train_ks.txt'),\
                                           sep=" ",\
                                           header=None,\
                                           names=['ImageName', 'label'])

        if 'val' in get_list:
            self.val_anno = pd.read_csv(os.path.join(self.anno_root, 'train_ks.txt'),\
                                           sep=" ",\
                                           header=None,\
                                           names=['ImageName', 'label'])
